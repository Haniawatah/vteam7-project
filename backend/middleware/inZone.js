...
export function randomStationLocation(stationZone) {
    if (!stationZone) return false;
    ...
    //Returnar ett random värde inne i bordern
    const lat = xMin + Math.random() * (xMax - xMin);
    const lng = yMin + Math.random() * (yMax - yMin);
