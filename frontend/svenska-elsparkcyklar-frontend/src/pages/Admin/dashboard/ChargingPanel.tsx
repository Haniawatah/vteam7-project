...
import { fetchChargingStationsAdmin } from '../../../services/admin';
import { useNavigate } from 'react-router-dom';
import { unChargingScooter } from '../../../services/scooters';
...
const StationsPanel: React.FC = () => {
  ...
  const refresh = async () => {
    ...
    setError(null);
    try {
      const data = await fetchChargingStationsAdmin();
  ...
  const addBike = (stationId: string) => {
    ...
    navigate(`charge/add/${stationId}`);
  };

  const removeBike = (scooterId: string, stationId: string) => {
    unChargingScooter(scooterId, stationId);
    refresh()
  };
